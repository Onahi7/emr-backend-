...
export class RoomsService {
  ...
  async create(createRoomDto: CreateRoomDto): Promise<Room> {
    const existing = await this.roomModel.findOne({ name: createRoomDto.name }).exec();
    if (existing) {
  ...
  async update(id: string, updateRoomDto: UpdateRoomDto): Promise<Room> {
    ...
    if (updateRoomDto.name) {
      const dup = await this.roomModel.findOne({ name: updateRoomDto.name, _id: { $ne: id } }).exec();
  ...
  async assignRoom(visitId: string, roomId: string): Promise<{ room: Room; visit: Visit }> {
  ...
  async releaseRoom(roomId: string): Promise<Room> {
  ...
  async autoAssignRoom(visitId: string, preferredType?: string): Promise<{ room?: Room; visit: Visit }> {
    const visit = await this.visitModel.findById(visitId).exec();
    if (!visit) throw new NotFoundException('Visit not found');

    // If visit already has a room, return it
    if (visit.room) {
      const existingRoom = await this.roomModel.findOne({ name: visit.room }).exec();
    ...
    const availableRoom = await this.roomModel
      .findOne({ roomType: typeToUse, status: RoomStatusEnum.AVAILABLE })
      .sort({ name: 1 })
      .exec();
    ...
    if (!availableRoom) {
      ...
      const anyRoom = await this.roomModel
        .findOne({ status: RoomStatusEnum.AVAILABLE })
        .sort({ name: 1 })
        .exec();
      if (!anyRoom) {
        return { visit };
      }
      return this.assignRoom(visitId, anyRoom._id.toString());
    }

    return this.assignRoom(visitId, availableRoom._id.toString());
  }

  async seedDefaultRooms(): Promise<number> {
    const existing = await this.roomModel.countDocuments().exec();
    ...
    await this.roomModel.insertMany(defaults);
