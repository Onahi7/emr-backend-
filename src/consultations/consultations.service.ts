...
import { branchFilterOptional, withBranch } from '../common/utils/branch-scope';
...
export class ConsultationsService {
  ...
  async update(id: string, updateConsultationDto: UpdateConsultationDto, branchId?: string): Promise<Consultation> {
    const consultation = await this.consultationModel.findOne({ _id: new Types.ObjectId(id), ...branchFilterOptional(branchId) });
    if (!consultation) {
      throw new NotFoundException('Consultation not found');
    }
